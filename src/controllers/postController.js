...
const deletePost = async (req, res) => {
    ...
    const deletedPost = await postServices.deletePost(parseInt(id, 10));
    if (!deletedPost) res.status(404).json({ message: 'Post does not exist' });

    res.status(204).end();
};

const findPostByQuerry = async (req, res) => {
    const { q } = req.query;
    console.log(q);
    const post = await postServices.findPostByQuerry(q);

    res.status(200).json(post);
};

module.exports = { createPost, getPosts, getPostById, updatePost, deletePost, findPostByQuerry };
